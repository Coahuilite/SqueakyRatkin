...
- [x] 0.3.1（2026-08-21）**离线面全部完成**（提交链 C20–C29：内核自足、race-aware、srdiag v2、年龄/fallback/彩蛋运行时、XML ABI 定型、验证配置全套；verify-local 9 项全绿，双语料 3782+10404 例回放零 delta；详情 MEMORY 0.3.1 条目与决策文档 §5）
- [ ] 0.3.1 **实机面验证**（§5 验证门，维护者执行）：迁移幂等（fixture 加载→迁移→再加载不再变）/重启/失败不丢（事务性）；immediate publish、350 ms 合并保存、close flush；No-DLC dormant 降级（不碰 Xenotype 路径）；UI 行集合=={Ratkin}、catalog 非装配域条目==0；BabyFits 实机只产新 action outcome、不产 MentalBreak outcome；fallback 删 pack → 内置表发声、17 action 逐一路由（Crying/Giggling 无内置条目走静默）；egg 开关关/开对比。完成后发布流程（runbook）与 Claim Pack。
