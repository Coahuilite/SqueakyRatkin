...
- [x] 0.3.0 三项发布决策定案（2026-08-20）：① 双轨 = dev CI 打包产物试用（不设 GitHub prerelease tag，维护者自选渠道分发，观察 2 周起）；② 措辞 = 内核重构 + 拆分预告（不提拆分模组名称，只预告独立前置 mod，草案见决策文档 §5）；③ 热修 = `vX.Y.Z-hotfixN` 方案 a（z 不变，GitHub 自动 prerelease 标记为已知代价，下个 z 正式合并）。双轨发布执行仍阻塞（merge 授权 + 渠道发布授权）。
