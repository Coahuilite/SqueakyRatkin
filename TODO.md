...
- [ ] **Steam 上传（待阻断解除）**：暂存包 `dist/steam/SqueakyRatkin` 已重建（116 文件，`build=steam`/`commit=cd90a9e`）。步骤 = 粘贴 [`steam-workshop-page-copy-draft.md`](docs/steam-workshop-page-copy-draft.md) 的中英正文与 Change Notes（含 R6 修复条目）→ 维护者人工上传（同 item Update，非 SteamCMD）→ 页面只读核验 → 更新 Claim Pack 的 Workshop 渠道格。
