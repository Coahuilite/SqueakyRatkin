...
- [ ] 0.3.0：纯枚举按领域归属提取 + `ActionKey` 边界映射（内核统一字符串动作键）；`Kernel/` 骨架（域键/池/链/调制/内置表）；`tools/KernelCharacterization`（纯度门+单测+黄金语料）；resolver 接内核（注入 `(Ratkin,*)`）；同变更删 `ChoosePack`/`vanilla` 字典/`Or`/`ResolvedAudioPack`；语料固化（详见决策文档 §5）。
