...
## 文档终审发现（未修复）
- [ ] 修复三层 Remix 在缺失前层时误选 None 的问题，处理冻结语料兼容，并验证实际游戏路径；公开 Select 入口已复现，见 终审 R6。
- [ ] 裁决 pawn label 的日志隐私/兼容政策、XML ABI 精确发布锚点；其余迁移与发布待办见维护入口。

> **当前状态（2026-09-19）**：0.3.3 本地就绪（本地提交领先 `origin/dev`，**未推送**）；dev 包已产出 `dist/dev/SqueakyRatkin-dev-v0.3.3-92be4d6.zip` 且**维护者实机测试通过**（Player.log 零 SR 错误），发布推进待授权；`kiiro-experiment` 分支已删（本地 + 远端）；**文档收敛已完成**（活跃 docs 10 份、自足；`docs/archive/` 与全部 `codemap.md` 已删除，证据保留在 `docs/release_review/`）；维护入口 `docs/maintenance-status-zh.md`；发布流程入口 `docs/release-runbook-zh.md`（发布路径 = 大版本分支 → main）。
...
- [x] 起草「内部普遍化设计笔记」（`docs/archive/internal-universalization-design-note-zh.md`）：冻结 race 发现/域模型/装配边界/池路由/迁移顺序与六项物理拆分门；该笔记是规划输入，不覆盖现行合同。
- [x] **0.3.x 架构决策（2026-08-18，分支 `0.3.x`）**：四方案并行比拼（内核但薄 / 契约先行分层 / 数据驱动极简 / 双内核渐进替换），采纳「内核但薄」——决策逻辑进零 Verse 引用 `Kernel/` 编译集 + `tools/KernelCharacterization` 黄金语料，不拆程序集；吸收契约派的迁移事务性/UI 泄漏点修复/BabyFits 源码核验 hook，与对拍派的黄金语料升级。实施唯一入口：`docs/archive/0.3x-refactor-architecture-decision-zh.md`。
...
- [ ] **0.3.2/0.3.3（2026-08-22 重排；2026-08-23 定案：0.3.2 正式版跳过，全部工作随 0.3.3 发布）＝功能调整 + XML ABI 固化 + Eat 两级开关**（SR 0.3.x UI 不变，原 UI 专项转 US 仓库；ABI 文档按「首个携带 0.3.1 XML ABI 的发布版本」表述）：
  ...
  - [x] **文档收敛与独立化（2026-09-19 完成）**：外部 agent 完成收敛（活跃 docs 10 份自足）；本会话完成独立化（去 archive 引用）、archive 整体删除、全部 14 份 `codemap.md` 删除、`AGENTS.md` 路径修订（证据位置改为 `docs/release_review/`）；证据与终审报告保留在 `docs/release_review/`，删除内容可从提交 `8481cbe` 取回；**不新增检查门**。
  - [ ] 证据退役（**延后，下次再删**）：`docs/release_review/` 的 7 份 Claim Pack + 2 份流程复盘 + 终审报告在本轮保留；下一次证据退役时再评估删除或压缩。
  - [ ] 收敛遗留（未做/待办）：① **记忆压缩未落地**——TODO 仍大量 `[x]`、MEMORY 含历史叙述、`OBLIVIONIS.md` 未新增段落（AGENTS.md 驱动条目要求）；② 终审 R6：Remix 在「无 Xenotype 层 + Race + BuiltIn」时可能选空层（已用公开 Select 入口编译复现，未修）；③ R4/R5 日志 label 与旧隐私禁令冲突仍开；④ R3「首个携带 0.3.1 ABI 的发行版本」未精确到 tag。
  - [x] **流程简化裁决 V1–V3（2026-09-13 定案）**：V1 追认本地 commit 免授权；V3 接受 CI 加门 + SDK 对齐 10.0.x；V2 先出专项方案不动 git——[`docs/privacy-history-rewrite-plan-zh.md`](./docs/privacy-history-rewrite-plan-zh.md)（方案 B 执行需单独授权；Q1–Q5 待裁决）。
...
- [ ] **US 兼容性（SR 退役为纯音频包前的硬前置，2026-08-23 检查完成）**：技术报告 `docs/archive/us-sr-compatibility-check-zh.md`；**迁移/退役方案 `docs/archive/us-sr-migration-plan-zh.md`**。当前双开安全（US 只承认 US 型包）；**风险**：US 自动装配逃逸门只认自己程序集的 comp 类型，任一 US 型 Ratkin 包或 legacy 桥落地 → Ratkin 双 comp → 双响（两仓 15/16 patch 同目标）。**归属与顺序硬门（存量玩家约束）**：修复落 US 侧（U1 跨程序集检测 + 让位 + skip 日志），SR 本窗口零运行时改动；顺序 = `U1 落地 → US 型 Ratkin 包/桥 → SR 1.0 内容化`，倒置即事故（先包/桥=双响；SR 先退役=静默）。待办：① 转述 US 侧 U1–U4；② 冻结 `SqueakyRatkin.CompProperties_Squeaker`/`CompSqueaker` 全名与 XML 装配契约；③ 1.0 前置链 SR → US → FerriteLib 且 SR 1.0 不编译引用 US 类型（缺前置=警告+静默，不报错）；④ legacy 桥类型名重叠期归属；⑤ 报告 §5 双开矩阵纳入两侧发布门。
...
- [x] **Eat 两级选项 → 已定案并实施（2026-08-23 维护者）**：父「仅真正进食」+ 子「使用成瘾品」（方案 B，`ChewIngestible`，toil 名未确认时回落完整 job 级）；规格与验收矩阵见 `docs/archive/handoff-eat-occurrence-granularity-zh.md` §5.1/§6/§7.1，实施与实机项见上方 0.3.2 列表。
