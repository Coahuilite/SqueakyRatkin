...
- `SqueakProductDomainFilter.cs` — 0.3.x 版本限定产品域过滤器（0.4.x 移除）：白名单 `{Ratkin}` 集中一处，catalog 构建、池/内置装配、投影三入口强制经过；`Contains(raceDefName)`/`ToKernelFilter()`。
...
**数据模型 / 杂项**
- `SqueakVoicePackModels.cs` — `SqueakVoicePackScope`（Unspecified/Race/Xenotype）、`SqueakVoicePackMode`（Off/Fallback/Remix）、`SqueakVoicePackDef`（一个 Def = 一个域一个可选包，只带音频不带行为/mood；`raceDefName` 0.3.1 必填路由声明）、`SqueakVoicePackAction`、`SqueakVoicePackValidator`（纯生产音频契约，Def 校验与候选准入共用；`raceDefName` 硬要求）、`VoicePackSelectionRecord`（canonical last-wins 持久化选择，`ComposeDomainKey(scope, target)` 域键）、`SqueakVoicePackDomainStatus`。
