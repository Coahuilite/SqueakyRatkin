...
- `SqueakyRatkinSettings.cs` + `SqueakyRatkinSettings.ExposeData.cs`（partial）— 主文件保留全部运行时桥和 public API；Scribe partial 集中 schema 4/2、`ExposeData`、`migrationPersistencePending`、隐藏 `experimentalRaceAllowlist` 与 v3/v1→v4/v2 临时 clone 事务。成功后才替换 records/bump schema/标记持久化；失败保留原列表/旧 schema，由主线程既有保存队列唯一写入 `base.WriteSettings()`。
...
**运行时快照发布（单发布者、不可变）**
- `SqueakRuntimeResolver.cs` — 静态单例快照发布器：`InitializeMainThread()`（主线程绑定，唯一发布者）、`NotifyContinuousResolverChange`（75 ms trailing / 150 ms bounded 防抖）、`NotifyDiscreteResolverChange`（返回前生效）、`TickPendingRuntimeChanges`/`FlushPendingRuntimeChanges(force)`、`TryPublish`→`BuildSnapshot`；只读面 `SqueakRuntimeSnapshot`（`ResolveContext(pawn)` 按 xenotype defName 查找、`ChooseProductionSound`（域身份 = pawn 真实 race + canonical xenotype）、`KnownMapSoundDefs`、`VoicePackMode`）、`ResolvedSqueakContext`、`RuntimeActionDelta`/`RuntimeMoodDelta`、`SqueakSoundChoice`。计数器 `ResolverRebuildCount`/`RuntimeFlushCount`。
...
- `SqueakXenotypeCatalog.cs` — volatile 目录快照：`Refresh()`（收集合法且 pack key 唯一的 `SqueakVoicePackDef`，经 `SqueakProductDomainFilter` 域闸 → `RacePacks`/`XenotypePacksByDefName` → canonical XenotypeDef **只登记装配域**（HAR Ratkin 限定集 ∪ 声明 pack 的目标；重复 defName 进 `AmbiguousCanonicalDefNames`，运行时 fail-closed）→ HAR hints）；`SqueakXenotypeCatalogSnapshot`（`XenotypeByDefName`/`PackByKey`/`RacePacks`/`XenotypePacksByDefName`/`GetTargetCandidates`（0.3.1 assembled-only 投影：declared_pack+selection+preset，HAR 仅 dev 诊断）/`GetVoicePackDomainPacks`）。Race 域与 Biotech 步骤完全独立。
- `SqueakProductDomainFilter.cs` — 0.3.x 版本限定产品域过滤器（0.4.x 移除）：静态默认 `{Ratkin}`；隐藏 `experimentalRaceAllowlist` 非空时完全替换名单；catalog `Contains(..., settings)` 与 resolver `KernelFilterFor(settings)`共用同一 Settings 输入，Kernel 本身不引用此适配层。
- `HarRatkinXenotypeDiscovery.cs` — 可选 HAR 适配器：全反射（HAR 非构建依赖），Biotech + `ThingDef "Ratkin"` 存在才探测；仅产 dev 诊断候选，**不是** VoicePack 资格门，0.3.1 起不进 UI 投影。

**触发与播放（每 pawn）**
- `CompSqueaker.cs`（946 行）— 核心 `ThingComp`：`CompTick`（周期采样）、`Notify_Wounded/Select/Death/Draft/Attack/Equip/MentalBreak`（外部事件）、`NotifyPeriodicDespawn`、`PostSpawnSetup`（成员注册 + 周期启动锚点 `startupAnchorTick`）、`TryTrigger`（完整触发管线）、`NotifyExternal`（外部触发闸层：spawned/map/视口/plan.Configured + **0.3.2 身份门控**——玩家发起来源要求 `IsPlayerControlled`，点选另要求 `!Downed && Awake()`，fail-closed 静默）、`PlayOneShot`、`PreviewFinal`（预览专用，无门/冷却/状态，`Rand.PushState/PopState`）、`GetDiagnosticSnapshot`/`ResetDiagnosticState`、`SqueakTimingModel`（纯时序求值）、静态运行时字段 `ScaleCooldownWithTimeSpeed`/`ScaleFrequencyWithTalking`/`ScalePeriodicWithAudiblePopulation`/`EatOnlyDuringChewing`/`EatIncludeDrugs`/`GlobalCooldownMultiplier`/`DiagnosticsEnabled`/`activeDistanceRange`、`ApplyDistanceRange`（运行时改写 `subSound.distRange`）；`IsEating` 经纯规则 `SqueakEatOccurrence.ResolveMode/AllowsOccurrence` 三态判定：父关 = 整段 `JobDefOf.Ingest` job（含端食物赶路）、父开 = vanilla `IEatingDriver.GainingNutritionNow`（零营养成瘾品不算）、父开 + 「使用成瘾品」= vanilla `ChewIngestible` toil（`SampleChewingToil`；toil 名本进程未确认时回落完整 job 级）；同文件还定义枚举 `SqueakMood`（Good/Neutral/Bad/Break）、`SqueakCooldownClock`、`SqueakTriggerMode`、`SqueakActionConfig`/`SqueakDistancePresetConfig`/`CompProperties_Squeaker`（XML 默认层：`globalMinIntervalTicks=216`、`scaleFrequencyWithTalking=true`、actions/moodMods/distancePresets）、诊断快照结构。
- `SqueakActionModel.cs` — `SqueakActionDefinitions`（17 个内置动作的唯一元数据源：`DisplayKey`/`AudioKey`/`VocalGatePolicy`/`SupportedScopes`/`DefaultScope`；0.3.1 波 3c append Crying/Giggling 15/16，无内置 SoundDef = 默认静默）、`SqueakActionPlan`（Configured/Mode/MinIntervalTicks/ProbabilityPerCheck/CooldownClock，`Unconfigured` 默认 300 tick/0.02）、`SqueakTriggerInvocation`（Origin/Source；非周期跳过 RandomOneShot 概率；0.3.2 派生 `IsPlayerInitiated`/`RequiresResponsivePawn`）、`SqueakVocalGatePolicy`/`SqueakActionScope`/`SqueakActionScopeSupport`。
...
- **门禁分层**：外部事件先过 `NotifyExternal` 静态闸（spawned/map/视口/plan.Configured）+ 0.3.2 身份闸（玩家发起来源需 `IsPlayerControlled`，点选另需 `!Downed && Awake()`；失败静默不耗冷却）；进 `TryTrigger` 后：全局作用域（`SqueakGlobalActionPolicy`，无需任何下游依赖、最先判定）→ xenotype `RuntimeActionDelta`（enabled/scope/interval/probability multiplier）→ 周期人口缩放 → `SqueakTimingModel`（per-action 间隔 × master × overall × action 倍率、时间倍速缩放、GameTicks/Realtime 冷却钟、全局冷却、出生锚定的启动相位）→ RandomOneShot 概率（外部触发跳过）→ vocal 门（发声器官效率 + Talking capacity）→ 播放。每条失败路径都 `RecordOutcome` + 统计埋点。
- **音源分层选择**：`ChooseProductionSound` — Off → 仅 vanilla `SR_*`；Fallback/Remix → 内核链 xenotype packs → race packs → pack fallback → 内置表，每层内 `gate.Playable` 过滤后带权抽 pack → 等权抽 sound；`ChainResult` 携带 `IsEgg`（0.3.2 日志装配）；`PoolStableKey` 供池稳定标识。
...
Patches Notify_*（wounded/select/death/draft/…）──┤→ TryTrigger
  闸门: spawned + MapHeld==CurrentMap + 视口ExpandedBy(10) + plan.Configured
        + 身份闸(0.3.2: 玩家发起→IsPlayerControlled; 点选→!Downed && Awake(); 失败静默)
...
- **→ Patches/**（[Patches/codemap.md](Patches/codemap.md)）：事件 patch 全部扇入 `CompSqueaker.Notify_*`/`NotifyPeriodicDespawn`（wounded/attack/death/select/draft/equip/mentalBreak/mentalFit）、`SqueakPeriodicPopulation.RemoveMap`（地图销毁）、`SqueakRuntimeResolver.FlushPendingRuntimeChanges` 与 `SqueakyRatkinMod.NotifySettingsWindowClosing`（设置关闭）、`SqueakDiagnosticsOverlay` 生命周期；`Patch_ModMetaData_LocalizedMetadata` 读 `SqueakyRatkinMod.PackageId`。装配点在本目录 `Mod.cs`。
