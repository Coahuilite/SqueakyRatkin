...
| `Source/SqueakyRatkin/SqueakyRatkin.csproj` | `<Version>`（identity 唯一来源）、`<SqueakyBuildFlavor>`（默认 `Dev` → `SQUEAKY_<FLAVOR>` DefineConstants；Dev 另加 `SQUEAKY_EXPERIMENTAL` 编译门）、`<SqueakyInformationalVersion>`（覆盖运行时 informational；EXP 分支仅 Dev flavor 默认 `$(Version)-EXP`）、`<OutputPath>..\..\1.6\Assemblies</OutputPath>`、Release 下 `DebugType=none` |
