...
让鼠族(Ratkin)在吃饭、睡觉、受击、选中、移动、社交、娱乐、精神崩溃时,按心情状态(好/中/坏/崩溃)发出吱吱叫。摄像机缩放联动"凑近听",屏幕外的鼠鼠不发声以节省性能。
...
## 开发者菜单(开发者模式)
开发者菜单 → "Squeaky Ratkin":悬浮字开关 ×2、音频浏览器。
...
### Build Flavor(分发用)
`-p:SqueakyBuildFlavor=Dev|Steam|GitHub`,影响启动日志 banner(`[dev|steam|github]`)。运行时功能三态相同。
