...
固定 17 个动作是 `Call`、`Eat`、`Sleep`、`Wounded`、`Select`、`Move`、`Social`、`Joy`、`Death`、`Draft`、`Undraft`、`Attack`、`Work`、`Equip`、`MentalBreak`、`Crying`、`Giggling`。`Crying`/`Giggling` 是 append-only 序数 15/16；未由 VoicePack 声明时默认静默。

音频使用玩家主动选择的独立 VoicePack：**Off** 仅内置 fallback；**Fallback** 按 Xenotype pack → Race pack → pack 自带 fallback → 内置 profile → 无声回退；**Remix** 让当前可播放的 Xenotype、Race、已声明 pack fallback、内置 profile tier 等权（未声明 pack fallback 的动作保留既有 Xenotype/Race/内置抽取）。Xenotype 目标只能是精确且区分大小写的 `XenotypeDef.defName`，并且是可选增量。
