...
- **0.3.0 实施状态（2026-08-19）**：换链完成——resolver 经 `Kernel/SqueakPoolRegistry.Select`（适配层 `SqueakKernelAdapter` 注入 `(Ratkin,*)`），旧 `ChoosePack`/vanilla 字典/`Or`/`ResolvedAudioPack` 同变更删除；证据链 C1-C7（fixture 9 场景、枚举提取+ActionKey、Kernel 骨架、KernelCharacterization 41 断言+3782 例语料回放零 delta、等价评审、resolver 接入、fixture 驱动语料）；发布门槛离线面绿（检查表 `docs/0.3x-release-gate-checklist-zh.md`），实机面与三项发布决策待维护者回机。
