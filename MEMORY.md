...
- **0.3.1（2026-08-21 重排）＝全分离 + race/年龄/fallback/彩蛋运行时 + XML ABI 定型 + 全部验证配置**（决策文档 §5 详表；TODO 47）：内核自足收口（字符串键/`BuiltInActionKeys`/内核 mode/`ActionKey` 入核/`SqueakActionModel` 出 harness/迁移清单=`Kernel/`+0 根文件/Ratkin 字面量与 `ComposeDomainKey` 拆除）；race-aware（`raceDefName`+validator、catalog 闸、assembled-only 投影、事务性 Scribe 3→4/1→2、srdiag v2、试验名单、per-race 池实证）；年龄/fallback/彩蛋（ageTag 优先级、LifeStageResolver、ComposeModulation 接线、pack fallback tier、内置表+FallbackProfileStore、Crying/Giggling 17 动作、IsEgg 落地）；XML ABI 定型 + 合同提升；验证配置全套（五处同步断言、漏斗纯逻辑提取、语料 17 矩阵+彩蛋维度、Config harness、v2 characterization、Settings partial）。
- **0.3.2（2026-08-21 重排）＝UI 专项**：fallback 编辑器/重建按钮/浏览器下放、XenotypeUI Race 域编辑器、Debug 入口合并、三档缩放验证；动作门机制本体仍 0.4.x（TODO 48）。
- **本地一键校验（2026-08-20）**：`scripts/verify-local.ps1` 一次输入 = 三 harness + `fixtures/` 零 delta 门 + Dev/Steam 双 flavor 0 warning 构建（实测 12.7s 全绿；`-PackDev`/`-PackSteam` 追加打包；fail-fast + 单检查重跑命令）。手动纪律的自动入口，日常验证不需要 agent 会话。
