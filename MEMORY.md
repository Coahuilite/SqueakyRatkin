...
- `0.3.x` 架构决策（2026-08-18）：四方案并行比拼后采纳「内核但薄」——0.3.x 决策逻辑（域键/池/链/fallback/年龄/Filter）进零 Verse 引用 `Kernel/` 编译集 + `tools/KernelCharacterization`（纯度门+单测+黄金语料），不拆程序集；吸收迁移事务性、`GetTargetCandidates` assembled-only 投影、BabyFits hook 源码核验（`TryStartMentalState` 成功 postfix + `MentalFitDef` 反向 map）、黄金语料升级。实施唯一入口 [`docs/0.3x-refactor-architecture-decision-zh.md`](./docs/0.3x-refactor-architecture-decision-zh.md)；0.3.2 冻结 17 动作 ABI（Crying/Giggling append 15/16）与 Fallback 末端→profile→无声。
