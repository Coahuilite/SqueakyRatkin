# DEPRECATED — SqueakyRatkin Engineering Review — 2026-07-03

> **Historical review — do not use as current guidance.** Current architecture is defined by [`project-architecture-contract.md`](./project-architecture-contract.md); the original review follows for archaeology only.

> **Superseded for current implementation facts (2026-07-19).** This is a preserved historical review, not a description of the current worktree. Its findings about missing global cooldown, duplicated `IsRatkin` eligibility, gene-target expansion, and a future open action registry have since been addressed or intentionally rejected. See the current [architecture contract](./project-architecture-contract.md).
