...
一句话权衡：0.3.x 阶段门是"选择语义正确性门"（per-race 池、fallback、静音、年龄），游戏内验证慢、抖、不可复现；约 500 行内核 + 200 行 harness 把验收证据变成确定性单测与可回放语料。不拆内核技术上做得了，但阶段门证据质量是真实瓶颈。
...
- `Enums.cs`：`SqueakAction`/`SqueakMood`/`SqueakVoicePackMode`（自 CompSqueaker.cs/SqueakVoicePackModels.cs 机械提取，namespace 不变 `SqueakyRatkin`，存档按名序列化已证安全，零 using churn）
...
- 枚举 → `Enums.cs`（namespace 不变，存档透明）；`SqueakActionConfig`/`SqueakDistancePresetConfig`/`SqueakTriggerMode` 原地。
...
| 6 | 年龄 ABI 冻结失误（0.3.2 一锤定音） | ageTag field-presence（第三方包零迁移）+ 冻结前 XML 评审 + 单测锁优先级语义 + append-only 序数 |
