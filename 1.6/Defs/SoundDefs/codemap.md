...
Defs 加载（LoadFolders v1.6，无条件）
  ...
  → SqueakRuntimeResolver.BuildSnapshot：
      GetVanilla() 按 SqueakActionDefinitions.AudioKey 查 SR_<Action>（无则 null → 该动作无声）
      Race tier = 玩家选中的 Race 范围 pack（含 SR_OfficialExample_Race）
  → 触发时 SqueakRuntimeSnapshot.Choose(context, action, ...)：
      Off → 仅 Vanilla；Fallback → XenotypePack.Or(RacePack).Or(Vanilla)；Remix → 三层等权随机
...
- 与 `Patches/Ratkin_AddSqueakComp.xml` 的关系：patch 里 15 个 `<action>` 名决定本目录 `SR_<Action>` 必须存在；两者由 `SqueakAction` 枚举值隐式连接（非 XML cross-ref）。
